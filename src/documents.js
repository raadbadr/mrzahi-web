...
function collapseGlyphGaps(line) {
  /* المسافة بين الكلمات لا تمس: كانت كل مسافة بين حرفين عربيين تحذف، فتخرج
     «جلسة شركة الأبراج» كلمة واحدة ملتصقة. الكلمات في السطر المفرق تفصلها
     مسافتان فأكثر، فتقسم المجموعات عليها، ولا يلصق داخل المجموعة إلا ما كان
  ...
  /* المجموعة كلها شظايا حروف (حرف أو حرفان) → كلمة واحدة، كما تخرج ملفات
     PDF الرسمية؛ وإلا فكلماتها سليمة ولا يلصق فيها إلا سلسلة الحروف المفردة. */
...
function findDate(text, labels) {
  ...
  if (dateParts[0].length === 4) return { raw: `${dateParts[0]}-${dateParts[1]}-${dateParts[2]}`, year: Number(dateParts[0]), label: dateMatch[0] };
  return { raw: `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`, year: Number(dateParts[2]), label: dateMatch[0] };
}

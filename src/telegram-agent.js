/* وكيل مستر زاهي على تيليغرام: نموذج لغوي يستعمل أدوات MCP نفسها (بحث، مواعيد، إضافة، إنجاز، إسناد) باسم المستخدم المرتبط،
...
const AGENT_TOOLS = TOOLS.filter((t) => ["mrzahi_company", "mrzahi_items", "mrzahi_search", "mrzahi_list", "mrzahi_add", "mrzahi_complete", "mrzahi_assign", "mrzahi_update", "mrzahi_team", "mrzahi_remind", "mrzahi_expenses", "mrzahi_overview", "mrzahi_platform"].includes(t.name));
...
function systemPrompt(ctx) {
  ...
  return [
    `أنت مساعد مستر زاهي الذكي داخل تيليغرام. تخاطب الآن: ${ctx.name || "مستخدم مرتبط"}${ctx.orgName ? ` من شركة «${ctx.orgName}»` : ""}. أنت تعرفه وتعمل على حسابه بصلاحياته، فلا تسأله من هو ولا تطلب ربطا.`,
    ...
    `طلب تعديل عنصر قائم (عدّل موعد…، أجّل…، غيّر عنوان…، بدّل العميل…) = mrzahi_update بالعنصر كما سماه وبالحقل الذي يتغير وحده؛ الموعد الجديد يمرر تاريخا كاملا بصيغة ISO محسوبا من كلامه («25-09» = يوم 25 من الشهر الجاري أو التالي إن مضى، «بعد أسبوع» = من اليوم).`,
    `حين يطلب إضافة أو إنجاز أو إسناد أو تعديل بصيغة واضحة نفذ بالأداة مباشرة ثم أخبره بما تم بعنوان العنصر (لا برقمه القياسي). إن كانت المهمة بلا قضية أو مخالفة تنتمي إليها فاعرض المرشحين الذين تعيدهم الأداة واطلب اختيار واحد. إن كان الطلب غامضا اسأل سؤالا واحدا قصيرا.`,
    `الإنجاز والإضافة والإسناد والتعديل لا تكون إلا بطلب صريح في رسالة المستخدم الحالية (أنجزت، أقفل، أضف، أسند، عدّل، أجّل…)، ولا ينفذ شيء قبل أن يؤكد بزر. كلمات المجاملة والتعليق (أحسنت، شكرا، ممتاز، تمام) ليست أوامر: رد عليها بجملة قصيرة فقط ولا تلمس أي عنصر.`,
    ...
    `لوحة التحكم: https://mrzahi.com/app/dashboard.html — المستندات: https://mrzahi.com/app/documents.html`,

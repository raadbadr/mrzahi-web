...
const TEXT = {
  ar: { reminder: (t, due, tr) => `⏰ تذكير من مستر زاهي\n${t}\nالاستحقاق: ${due}${tr ? `\nالسجل: ${tr}` : ""}\nhttps://mrzahi.com/app/dashboard.html`,
        ...
        missYou: (name) => `وحشتنا${name ? ` يا ${name}` : ""} 👋`,
        missYouDays: (n) => `لم تفتح حسابك منذ ${n} ${n === 1 ? "يوم" : n === 2 ? "يومين" : n <= 10 ? "أيام" : "يوما"}.`,
        ...
        trialCall: "اشترك الآن كي لا يتوقف شيء.",
  ...
  en: { reminder: (t, due, tr) => `⏰ MrZahi reminder\n${t}\nDue: ${due}${tr ? `\nTracker: ${tr}` : ""}\nhttps://mrzahi.com/app/dashboard.html`,
        ...
        test: "✅ Test message from MrZahi: this channel works.",
        missYou: (name) => `We missed you${name ? `, ${name}` : ""} 👋`,
        missYouDays: (n) => `You have not opened your account for ${n} ${n === 1 ? "day" : "days"}.`,
        ...
        trialCall: "Subscribe now so nothing stops.",
        trialBtn: "Plans and subscription" },
  fr: { reminder: (t, due, tr) => `⏰ Rappel MrZahi\n${t}\nÉchéance : ${due}${tr ? `\nSuivi : ${tr}` : ""}\nhttps://mrzahi.com/app/dashboard.html`,
        linked: (name, org) => `Bonjour ${name || ""}${org ? "\nSociete : " + org : ""}`,
        badCode: "Code invalide ou expiré. Ouvrez Paramètres dans MrZahi et copiez un nouveau code.",
        needCode: "Pour lier votre compte : ouvrez Paramètres dans MrZahi → Telegram → « Générer un code », puis envoyez le code ici ou scannez le QR code.",
        alreadyLinked: (name) => `Compte lie${name ? ", " + name : ""}.`,
        test: "✅ Message de test MrZahi : ce canal fonctionne.",
        missYou: (name) => `Vous nous avez manqué${name ? `, ${name}` : ""} 👋`,
        missYouDays: (n) => `Vous n'avez pas ouvert votre compte depuis ${n} ${n === 1 ? "jour" : "jours"}.`,
        ...
        trialCall: "Abonnez-vous pour que rien ne s'arrete.",
        trialBtn: "Offres et abonnement" },
  ur: { reminder: (t, due, tr) => `⏰ MrZahi یاد دہانی\n${t}\nآخری تاریخ: ${due}${tr ? `\nرجسٹر: ${tr}` : ""}\nhttps://mrzahi.com/app/dashboard.html`,
        linked: (name, org) => `خوش آمدید ${name || ""}${org ? "\nکمپنی: " + org : ""}`,
        badCode: "کوڈ غلط یا ختم ہو چکا ہے۔ MrZahi کی ترتیبات کھول کر نیا کوڈ کاپی کریں۔",
        needCode: "اکاؤنٹ منسلک کرنے کے لیے: MrZahi کی ترتیبات ← ٹیلیگرام ← «کوڈ بنائیں»، پھر کوڈ یہاں بھیجیں یا QR کوڈ اسکین کریں۔",
        alreadyLinked: (name) => `${name ? name + "، " : ""}آپ منسلک ہیں۔`,
        test: "✅ MrZahi سے آزمائشی پیغام: یہ چینل کام کر رہا ہے۔",
        missYou: (name) => `ہمیں آپ کی کمی محسوس ہوئی${name ? `، ${name}` : ""} 👋`,
        missYouDays: (n) => `آپ نے ${n} دن سے اپنا اکاؤنٹ نہیں کھولا۔`,
        ...
        trialCall: "ابھی سبسکرائب کریں تاکہ کچھ نہ رکے۔",
...
const BOT = {
  ...
  en: { company: "🏢 Company", upcoming: "📅 Upcoming", overdue: "⏰ Overdue", dashboard: "🌐 Dashboard", help: "❓ Help",
        ...
        voiceHeard: "🎙 I heard: ", fileUnreadable: "I could not read this file. Send a PDF, image, Excel or CSV.", fileTooBig: "The file exceeds the allowed size (15 MB).", fileQuestion: "Summarize this file briefly and say what the user can do with it in MrZahi (for example import it from the Import page if it is a table of due dates, violations or cases).",
        linkIntro: "Welcome to MrZahi 👋\nTap the button to link this bot to your account: the settings page opens and the link completes automatically.",
  ...
  ur: { company: "🏢 کمپنی", upcoming: "📅 آنے والی تاریخیں", overdue: "⏰ تاخیر شدہ", dashboard: "🌐 ڈیش بورڈ", help: "❓ مدد",
        ...
        voiceHeard: "🎙 میں نے سنا: ", fileUnreadable: "یہ فائل پڑھی نہیں جا سکی۔ PDF، تصویر، ایکسل یا CSV بھیجیں۔", fileTooBig: "فائل اجازت شدہ حد (15 MB) سے بڑی ہے۔", fileQuestion: "اس فائل کا مختصر خلاصہ کریں اور بتائیں کہ صارف MrZahi میں اس کا کیا کر سکتا ہے (مثلا اگر یہ تاریخوں، خلاف ورزیوں یا مقدمات کا جدول ہے تو درآمد صفحے سے درآمد کریں)۔",
        linkIntro: "MrZahi میں خوش آمدید 👋\nاس بوٹ کو اپنے اکاؤنٹ سے منسلک کرنے کے لیے بٹن دبائیں: ترتیبات کا صفحہ کھلے گا اور منسلکی خود بخود مکمل ہو جائے گی۔",

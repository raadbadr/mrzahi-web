...
const SMALL_TALK = /^(كيف حالك|كيفك|شلونك|اخبارك|وش اخبارك|من انت|مين انت|انت مين|ايش تسوي|وش تسوي|ماذا تفعل|ايش تقدر تسوي|وش تقدر تسوي|ماذا تستطيع|قدراتك|مساعده|ساعدني|help|who are you|what can you do|how are you|aide|qui es tu|مدد)( .*)?$/;
const ABOUT_REPLY = { ar: "أنا مساعد مستر زاهي، أجيب من بيانات حسابك وحدها ولا أخمن شيئا. اكتب: قضايا، مخالفات، مهام، مستندات، مواعيد، متأخر، مصاريف، الشركة، الفريق.", en: "I am the MrZahi assistant. I answer only from your account data and never guess. Try: cases, violations, tasks, documents, upcoming, overdue, expenses, company, team.", fr: "Je suis l'assistant MrZahi. Je réponds uniquement à partir de vos données et ne devine jamais. Essayez : affaires, infractions, tâches, documents, échéances, retards, dépenses, société, équipe.", ur: "میں MrZahi کا اسسٹنٹ ہوں۔ میں صرف آپ کے ڈیٹا سے جواب دیتا ہوں اور اندازہ نہیں لگاتا۔ لکھیں: مقدمات، خلاف ورزیاں، کام، دستاویزات، تاریخیں، تاخیر، اخراجات، کمپنی، ٹیم۔" };
...
export function understand(text, lang) {
  ...
  if (SMALL_TALK.test(n)) return { reply: ABOUT_REPLY[lang] || ABOUT_REPLY.ar, kind: "about" };
  const toks = n.split(" ");
  ...
  /* «المسجلين» و«التسجيل» أسماء أيضا لا فعل «سجل» */
  const noNouns = (sajilNoun ? raw.replace(/سجل(?:ات|نا|كم|هم)?/g, "") : raw).replace(/(?:ال)?مسجل(?:ين|ون|ه|ات)?|(?:ال)?تسجيل(?:ات)?/g, "");

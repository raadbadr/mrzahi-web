...
for (const name of pages) {
  for (const [w, h, label] of sizes) {
    ...
    if (process.env.PREP_JS) { try { const _r = await page.evaluate(process.env.PREP_JS); if (_r) console.log("PREP>>", JSON.stringify(_r)); await new Promise((r) => setTimeout(r, 250)); } catch (e) { errors.push("prep: " + e.message.slice(0, 120)); } }

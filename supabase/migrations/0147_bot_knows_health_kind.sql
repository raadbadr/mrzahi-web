-- 0147 — «صحتي» نوع يعرفه بوت تيليغرام وخادم MCP كما يعرف القضايا والمهام:
-- ادوية وعلاجات ومواعيد طبية ونادٍ وتمرين، بتصنيفها او بسجلها. ويعاد معها
-- حقل repeat فيعرف الوكيل ان الجرعة التالية تفتح وحدها ولا ينشئها يدويا.
begin;
  ...
  from (
    ...
    where i.org_id in (select public.telegram_user_orgs(p_user_id))
      ...
      and (k = 'all'
        ...
        or (k = 'health' and (coalesce(i.category,'') ilike '%صح%' or coalesce(i.category,'') ilike '%دوا%' or coalesce(i.category,'') ilike '%علاج%'
                              or coalesce(i.category,'') ilike '%health%' or coalesce(i.category,'') ilike '%نادي%' or coalesce(i.category,'') ilike '%جيم%'
                              or coalesce(i.category,'') ilike '%تمرين%' or coalesce(i.category,'') ilike '%gym%'
                              or coalesce(t.name,'') ilike '%صحت%' or coalesce(t.name,'') ilike '%health%'))

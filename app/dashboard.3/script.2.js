      ...
      var TILE_ICONS = {
        ...
        money: "sar"   /* لا مسار: يرسم بقناع رمز الريال في paintTiles */
      };
      ...
      function paintTiles(values) {
        ...
        tiles.forEach(function (tile, i) {
          ...
          /* المال يرسم برمز الريال السعودي (قناع .sar-symbol) لا بعلامة الدولار
             (امر المهندس رعد 2026-09-20: «المفروض تستخدم رمز الريال»). */
          var iconBox = card.querySelector(".platform-stat-icon");
          var svg = iconBox && iconBox.querySelector("svg");
          var sar = iconBox && iconBox.querySelector(".sar-symbol");
          if (tile.icon === "money") {
            if (svg) svg.style.display = "none";
            if (!sar && iconBox) { sar = document.createElement("span"); sar.className = "sar-symbol"; iconBox.appendChild(sar); }
          } else {
            if (sar) sar.remove();
            if (svg) svg.style.display = "";
            var path = iconBox && iconBox.querySelector("path");

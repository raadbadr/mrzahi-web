    ...
    (function () {
      ...
      var VIEW_TYPES = {
        ...
        /* المبيعات واجهة بائع لا واجهة محام (امر المهندس رعد 2026-09-17: «المبيعات
           تحتاج اعادة تصميم، مو معقول كل الواجهات نحصل نفس الشي العام الخاص
           بالمحامين»): الصفقة لها مراحلها وقيمتها واحتمال اغلاقها، لا حالة
           «مفتوح/منجز» وحدها. */
        deals: { titleKey: "viewDeals", defaultCategory: "صفقة",
                 words: ["صفقة", "صفقات", "فرصة", "فرص", "عميل محتمل", "عرض سعر", "deal", "deals",
                         "opportunity", "lead", "leads", "quote", "affaire", "affaires", "opportunite",
                         "سودا", "سودے"] },
        ...
        invoices: { titleKey: "viewInvoices", defaultCategory: "فاتورة",
                    words: ["فاتورة", "فواتير", "مستحق", "مستحقات", "تحصيل",
                            ...
                            "رسید"] },   /* «بل» الاردية حذفت: تطابق حرف العطف العربي كلمة كاملة */
        ...
        html += '<div class="tlx" id="tlx" dir="' + (isRtl ? "rtl" : "ltr") + '" style="--timeline-fill-pct:100%;--tlx-days:' + daysInMonth + '" data-today-step="' + todayStep + '">' +
          ...
          '<div class="tlx-nav-row">' +
            ...
            '<div class="tlx-scroll" id="tlxScroll"><div class="tlx-range-wrap">' + ms + months +
              /* المدى يبقى كاملا كي تتطابق خريطة المقبض مع مواضع الاعمدة
                 والتعبئة، والوقوف عند «الآن» يفرض على القيمة لا على المدى. */
      ...
      function wireTimeline() {
        ...
        slider.addEventListener("input", function () {
          ...
          /* لا مستقبل في سجل النشاط: السحب يقف عند «الآن» (امر المهندس رعد) */
          if (v > todayStep) { v = todayStep; this.value = String(v); }
          var nearest = null;

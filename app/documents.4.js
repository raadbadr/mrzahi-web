    ...
    (function () {
      ...
      /* t وfmt مستقلتان تماما هنا: الاعتماد على translations/l من سكربت آخر
         كان يرمي ReferenceError عند أول ملف يرفع، فيموت الاستيراد كله قبل أي
         طلب شبكة. القراءة الآن عبر مساري آمن لا يرمي أبدا. */
      function t(key) {
        try {
          var currentLang = typeof lang === "function" ? lang() : "ar";
          var dict = (typeof translations !== "undefined" && (translations[currentLang] || translations.ar)) || null;
          if (dict && dict[key] != null) return dict[key];
          if (typeof translations !== "undefined" && translations.ar && translations.ar[key] != null) return translations.ar[key];
        } catch (e) { /* السكربت المشترك لم يحمل بعد؛ الرجوع للمفتاح نفسه */ }
        return key;
      }
      ...
      function readFile(file) {
        ...
        reader.onload = function () {
          try {
            let wb;
            if (ext === "json") {
              /* JSON: مصفوفة كائنات، أو {rows|items|data: [...]} — تتحول إلى ورقة فيمر بالمساري نفسه */
      ...
      /* أسطر مرشحة ليكون أحدها صف العناوين، تعرض على المستخدم حين لا نثق بالتخمين */
      function candidateHeaderRows(aoa) {
        const scanTo = Math.min(aoa.length, 15);
        const out = [];
        for (let i = 0; i < scanTo; i++) {
          const cells = (aoa[i] || []).map(cellText).filter(function (v) { return v; });
          if (cells.length) out.push({ row: i, preview: cells.slice(0, 4).join(" · ") });
        }
        return out;
      }

      /* forcedRow: فهرس صف العناوين صفر الأساس، يفرضه المستخدم من منتقي الصفوف
         حين لا يثق التخمين بنفسه. confident=false توقف الاستيراد عند خطوة
         الربط وتعرض المنتقي بدل الاستمرار بأعمدة "عمود 1، عمود 2..." بلا معنى. */
      function parseSheet(ws, forcedRow) {
        const aoa = XLSX.utils.sheet_to_json(ws, { header: 1, raw: true, defval: null, blankrows: false });
        let hi = -1;
        let best = -1;
        const scanTo = Math.min(aoa.length, 30);
        if (typeof forcedRow === "number" && forcedRow >= 0 && forcedRow < aoa.length) {
          hi = forcedRow;
          best = headerScore(aoa[hi], nextDataRow(aoa, hi));
        } else {
          for (let i = 0; i < scanTo; i++) {
        ...
        if (hi === -1) return { headers: [], rows: [], confident: false, candidates: [] };
        ...
        for (let c = 0; c < width; c++) {
          const raw = cellText(rawHeaders[c]);
          let name = raw || (t("columnN") + " " + (c + 1));
          if (!raw) fallbackCount++;
        ...
        /* واثقون إن حدد الصف يدويا، أو إن أغلب الأعمدة حملت نصا حقيقيا لا أسماء احتياطية */
        const confident = typeof forcedRow === "number" || (width > 0 && fallbackCount / width <= 0.3);
        return { headers: headers, rows: rows, confident: confident, headerRow: hi, candidates: candidateHeaderRows(aoa) };
      }
      ...
      function selectSheet(name, forcedHeaderRow) {
        ...
        const parsed = ws ? parseSheet(ws, forcedHeaderRow) : { headers: [], rows: [], confident: true, candidates: [] };
        state.headers = parsed.headers;
        state.rows = parsed.rows;
        state.headerCandidates = parsed.candidates || [];
        state.headerRow = parsed.headerRow;

        if (!state.headers.length || !state.rows.length) {
          setMsg(els.fileStatus, t("emptySheet"), "error");
          show(els.headerPicker, false);
        ...
        renderHeaderPicker(parsed.confident);
        guessMapping();
        renderPreview();
        renderMapping();
        renderRecordSelect();
      ...
      /* الصف لم يلتقط بثقة: تنبيه واحد + منتقي صفوف بديل، بدل ترك المستخدم
         أمام أعمدة "عمود 1، عمود 2..." بلا معنى. */
      function renderHeaderPicker(confident) {
        if (!els.headerPicker) return;
        show(els.headerPicker, !confident);
        if (confident) return;
        const keep = els.headerRowSelect.value;
        els.headerRowSelect.innerHTML = "";
        state.headerCandidates.forEach(function (c) {
          els.headerRowSelect.appendChild(new Option(fmt("headerRowOption", { n: c.row + 1, preview: c.preview }), String(c.row)));
        });
        const wanted = keep !== "" ? keep : String(state.headerRow || 0);
        if ([].some.call(els.headerRowSelect.options, function (o) { return o.value === wanted; })) els.headerRowSelect.value = wanted;
      }
      ...
      /* معاينة ثانوية صغيرة فقط: أول أعمدة قليلة وأسطر قليلة، بلا زحمة تكسر البطاقة */
      const PREVIEW_ROWS = 5;
      const PREVIEW_COLS = 12;
      function renderPreview() {
        const shownCols = Math.min(PREVIEW_COLS, state.headers.length);
        const extraCols = state.headers.length - shownCols;
        const head = "<thead><tr>" + state.headers.slice(0, shownCols).map(function (h) { return "<th>" + esc(h) + "</th>"; }).join("") + "</tr></thead>";
        const body = "<tbody>" + state.rows.slice(0, PREVIEW_ROWS).map(function (r) {
          return "<tr>" + state.headers.slice(0, shownCols).map(function (_h, i) { return "<td>" + esc(cellText(r[i])) + "</td>"; }).join("") + "</tr>";
        }).join("") + "</tbody>";
        els.previewTable.innerHTML = head + body;
        els.previewNote.textContent = fmt("previewNote", { shown: Math.min(PREVIEW_ROWS, state.rows.length), total: state.rows.length });
        if (els.previewMoreCols) {
          show(els.previewMoreCols, extraCols > 0);
          if (extraCols > 0) els.previewMoreCols.textContent = fmt("previewMoreCols", { n: extraCols });
        }
      }

      /* الحقول الأساسية تظهر مباشرة، والباقي تحت "تفاصيل إضافية" — الواجهة تنظم
         حسب حقولنا نحن لا حسب فوضى الملف الوارد. */
      function renderMapping() {
        els.mappingGrid.innerHTML = "";
        if (els.optionalMappingGrid) els.optionalMappingGrid.innerHTML = "";
        FIELDS.forEach(function (f) {
          ...
          const target = (f.required || !els.optionalMappingGrid) ? els.mappingGrid : els.optionalMappingGrid;
      ...
      function resetFlow() {
        ...
        state.analysis = null;
        state.result = null;
        var mainInput = document.getElementById("docFile");
        ...
        var up = document.getElementById("uploadCard");
      ...
      function wireEvents() {
        ...
        els.headerRowSelect.addEventListener("change", function () {
          if (state.workbook) selectSheet(state.sheetName, parseInt(els.headerRowSelect.value, 10));
        });
      ...
      /* عناصر الاستيراد تجمع مرة واحدة، وتطلب عند أول ملف حتى لو تعثر الإقلاع */
      function buildEls() {
        if (els && els.importFlow) return els;
        els = {
          loadingCard: null, unavailableCard: null, noOrgCard: null,
          importFlow: $("importFlow"),
          step1: null, step2: $("step2"), step3: $("step3"), step4: $("step4"), step5: $("step5"),
          fileStatus: $("fileStatus"),
          sheetRow: $("sheetRow"), sheetSelect: $("sheetSelect"), previewNote: $("previewNote"), previewTable: $("previewTable"),
          previewMoreCols: $("previewMoreCols"),
          headerPicker: $("headerPicker"), headerPickerMsg: $("headerPickerMsg"), headerRowSelect: $("headerRowSelect"),
          mappingGrid: $("mappingGrid"), optionalMappingGrid: $("optionalMappingGrid"), mappingStats: $("mappingStats"), unmatchedHint: $("unmatchedHint"),
      ...
      window.mrzahiImport = {
        ...
        accept: function (file) {
          buildEls();
          var flow = document.getElementById("importFlow");
